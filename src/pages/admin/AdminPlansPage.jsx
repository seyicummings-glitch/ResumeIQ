...
import { FEATURE_KEYS, FEATURE_LABELS } from '../../lib/subscriptionFeatures'
...
function PlanCard({ plan, onEdit, onConfigureLimits, onToggleStatus, onDelete, isTogglingStatus }) {
  return (
    <Card className="flex flex-col gap-3">
      ...
      <div className="mt-auto flex flex-wrap gap-2 pt-2">
        ...
        <Button variant="secondary" size="sm" onClick={() => onConfigureLimits(plan)}>
...
function LimitsModal({ isOpen, onClose, plan }) {
  const { data: limits, isLoading } = usePlanLimits(plan?.id)
  ...
  useEffect(() => {
    ...
    limits.forEach((entry) => {
      currentValues[`${entry.featureKey}_daily`] = entry.dailyLimit ?? ''
      currentValues[`${entry.featureKey}_monthly`] = entry.monthlyLimit ?? ''
    })
    reset(currentValues)
  }, [limits, reset])
  ...
  async function onSubmit(values) {
    const payload = FEATURE_KEYS.map((key) => ({
      featureKey: key,
      dailyLimit: values[`${key}_daily`] === '' ? null : parseInt(values[`${key}_daily`], 10),
      monthlyLimit: values[`${key}_monthly`] === '' ? null : parseInt(values[`${key}_monthly`], 10),
    }))
    await updateLimits.mutateAsync({ id: plan.id, limits: payload })
    showToast('Limits updated.', { tone: 'success' })
    onClose()
  }

  return (
    <Modal isOpen={isOpen} onClose={onClose} title={plan ? `${plan.name} — feature limits` : 'Feature limits'}>
      {isLoading ? (
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
          <p className="text-sm text-text">Leave a field blank for unlimited usage.</p>
          <div className="flex flex-col gap-3">
            {FEATURE_KEYS.map((key) => (
              <div key={key} className="grid grid-cols-3 items-end gap-3">
                <p className="text-sm font-medium text-text-h">{FEATURE_LABELS[key]}</p>
                <Input label="Per day" type="number" min="0" {...register(`${key}_daily`)} />
                <Input label="Per month" type="number" min="0" {...register(`${key}_monthly`)} />
              </div>
            ))}
          </div>
          <div className="flex justify-end gap-2 pt-2">
            ...
            <Button type="submit" isLoading={updateLimits.isPending}>
              Save limits
            </Button>
          </div>
        </form>
...
export default function AdminPlansPage() {
  ...
  const [limitsPlan, setLimitsPlan] = useState(null)
  const [deletingPlan, setDeletingPlan] = useState(null)
  const planForm = useForm()

  function openCreate() {
    setEditingPlan({})
    planForm.reset({ name: '', slug: '', description: '', monthlyPrice: '', yearlyPrice: '', displayOrder: 0 })
  }
  ...
  return (
    <div className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {data.items.map((plan) => (
            <PlanCard
              ...
              plan={plan}
              onEdit={openEdit}
              onConfigureLimits={setLimitsPlan}
      ...
      <LimitsModal isOpen={Boolean(limitsPlan)} onClose={() => setLimitsPlan(null)} plan={limitsPlan} />
      ...
      <ConfirmDialog
        ...
        title="Delete plan"
        message={`This permanently deletes "${deletingPlan?.name || ''}" and its feature limits.`}
