...
@patch("app.services.email_service.smtplib.SMTP")
def test_send_verification_email_builds_link_and_sends(mock_smtp_cls, monkeypatch):
    monkeypatch.setenv("SMTP_HOST", "smtp.example.com")
    monkeypatch.setenv("SMTP_USERNAME", "user@example.com")
    monkeypatch.setenv("SMTP_PASSWORD", "app-password")
    monkeypatch.setenv("FRONTEND_URL", "https://app.example.com")
    ...
    send_verification_email("someone@example.com", "abc123")
    ...
    sent_message = mock_server.send_message.call_args[0][0]
    assert sent_message["To"] == "someone@example.com"
    assert "https://app.example.com/verify-email?token=abc123" in sent_message.get_content()
